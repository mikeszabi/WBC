...
def segment_fg_bg_sv_kmeans(csp, init_centers='k-means++', n_clusters=4, vis_diag=False):  
...
def segment_hsv(csp, mask=None, init_centers='k-means++', cut_channel=1, chs=(0,1,2), n_clusters=3, vis_diag=False): 
    ...
    if vis_diag:
        ...
        for i, c in enumerate(center):
            ax.scatter(Z_rs[label_rs==i, chs[0]], Z_rs[label_rs==i, chs[1]], Z_rs[label_rs==i, chs[2]], color=colors[i,:])                   
    
            ax.scatter(c[chs[0]],c[chs[1]],c[chs[2]], 'o', s=100, c='k')
