...
def main(image_file,vis_diag=False):
    ...
    """                   
    hsv_resize, scale=imtools.imRescaleMaxDim(diag.hsv_corrected,param.middle_size,interpolation = 1)
    im_resize, scale=imtools.imRescaleMaxDim(diag.im_corrected,param.middle_size,interpolation = 1)
   ...
   # TODO: check distribution of 31
        ...
        for i, c in enumerate(cent_3):
            mask_tmps.append(morphology.binary_opening(label_3==i,morphology.disk(1)))
            n[i]=(mask_tmps[i]).sum()
#        
    ...
    mask_wbc_small_1=np.zeros(label_4.shape,'uint8') # NE AND EO
    mask_wbc_small_2=np.zeros(label_4.shape,'uint8') # EO AND MONO
    mask_wbc_small_3=np.zeros(label_4.shape,'uint8') # MONO and LYMPHO
    ...
    mask_wbc_small_0=np.zeros(label_4.shape,'uint8') # NE 
    area=np.zeros(len(regions))
    for i,r in enumerate(regions):
        area[i]=r.area
        if (r.convex_area>np.power((1.25*scale*param.rbcR),2)*np.pi):
    ...
    diag.saveDiagImage(im_pot,'potmask',savedir=diag_dir)
